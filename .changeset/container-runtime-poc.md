...
- `@vercel/container`: New builder that authenticates to VCR with the project's OIDC token, builds and pushes the image, and emits a digest-pinned reference as build output. Uses docker on developer machines and buildah (daemonless) in the Vercel build container, behind a shared `ContainerEngine` interface. Passes a permissive `registries.conf` to buildah so unqualified Dockerfile `FROM` lines resolve via docker.io without a TTY. The build flow is instrumented with tracing spans carrying non-secret diagnostics, with debug logging gated on `BUILDER_DEBUG` like other builders.
